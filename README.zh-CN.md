...
**Config** 提供取消配对和静音。选择底部 `<` 或 `>` 方块切换页面；到达目标页面后
同一方向按钮会保持选中，便于连续翻页。
...
按住右上角 `PAIR` 或 `LIVE` 状态区域 3 秒。进度必须达到 100%；提前松手会取消。
固件会断开当前主机、删除设备本地保存的全部绑定信息，并在不重启的情况下恢复广播。

出现 `UNPAIRED — FORGET ON HOST` 后，还需要在电脑蓝牙设置中忽略
**Codex Micro**。设备端无法删除 macOS 保存的蓝牙记录。
...
如果已在设备端取消配对，请在主机蓝牙设置中忽略 **Codex Micro**，然后重新配对。
只清除一端会导致绑定密钥不匹配，无法自动重连。
...
- 按住右上角连接状态 3 秒会删除设备保存的全部绑定信息，但不会修改主机保存的记录。
